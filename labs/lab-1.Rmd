...
You can create variables using the assignment operator (`<-`). Whatever is on the left of the assignment operator is saved to variable named whatever is to the left of the assignment operator. For example, if we wanted to save `10` to a variable called `x` we would write:

```{r}
my_number <- 10
```

If we want to see what is stored in `x`, we can simply enter `x` into the console.

```{r}
my_number
```

If we want to overwrite `x`, we simply assign a new value to `x`.
...
If we want to remove a variable from our environment, we use `rm()`.
...
If a variable only has one value associated with it, we call it a `scalar`. There are four types of scalars: doubles (e.g., `10.15`), integers (e.g., `10L`), characters (e.g., `"hello"`), and logicals (also called booleans) (e.g., `TRUE/FALSE`). At this point, it is not incredibly important to know the difference between the types of scalars. 

### Vectors

If a variable has more than one value associated with it, we call it a vector. We create vectors be using the concatenate function (`c()`). 

```{r}
my_vector <- c(10, 20, 20, 30, 40)
my_vector
my_vector[1]
my_vector[3] <- 999
my_vector
...
Functions are essentially pre-packaged snippets of code that make your life easier. A function takes some input (called arguments) and returns some values. For example, the `mean()` function takes a numeric vector as an argument and returns the average of the vector.

```{r}
(10 + 20 + 20 + 30 + 40) / 5

mean(c(10, 20, 20, 30, 40))
```

## Comments

Comments are pieces of text that are not interpreted by the computer. They are mostly used as a way to remind yourself what your code is doing or why you did something a certain way. In R we use an octothorp/pound-sign/hashmark/hashtag at the beginning of the line to denote a comment. In the example below, the first and third line of code would not not be evaluated while the second and fourth line would be.
...
If a function is a data science tool,then a package is a toolbox. 
