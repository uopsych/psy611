...
In R you will mostly be working with data frames. Data frames are lists of lists. It is not important to know what that means here, but understanding how to access values from within a data frame will be very important.

Let's look at a data frame built into R. Type `mtcars` to print out data from the 1974 Motor Trend US magazine. 

```{r}
mtcars
```

With data frames, you can extract a value using the `[row, col]` immediately after the object. For example, if we wanted to extract the number of gears (`gear`) in the `Mazda RX4` we could use `mtcars[1, 10]` to extract the value of the first row in the tenth column.

```{r}
mtcars[1, 10]
```

Since the rows and columns are named, we can also use the names.

```{r}
mtcars["Mazda RX4", "gear"]
```

If you want to extract an entire column, you can either not specify a row (i.e., `[, 10]`) 

```{r}
mtcars[, 10]
```

You can use extract columns using `$` followed by the column name.
...
Using the suite of `as.*()` functions: `as.numeric()`, `as.integer()`
...
To load the `rio` package and make its tools available to us, we use `library()`.

```{r}
library(rio)
```

*Note.* A package can be loaded with or without quotes: `library("rio")` **OR** `library(rio)`.
