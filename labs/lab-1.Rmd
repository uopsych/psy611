...
The purpose of today's lab is not teach you everything there is to know about coding in R. It is not even to describe why the code in R works the way it does. Instead it takes an applied approach to R. We hope to provide you with a functional understanding of R so that you may begin to play around with the language.
...
After we have discusssed the content of the lab, we will move onto [Minihacks](#minihacks).

***

# Getting Started {#gs}

R is a highly flexible and mostly intuitive open-source programming language used for data science. It is relatively new compared to other programming languages, but it has an engaged following of data scientists and developers.
...
If a vector has values of different types, we would use `list()` instead of `c()`. For example, if we want to create a vector with the values `10`, `"hello"`, and `TRUE` we would use `list(10, "hello", TRUE)`. We are not going to go too much into lists here. 


### Data Frames

In R you will mostly be working with data frames. Data frames are technically lists of atomic vectors, but it is not important to discuss that here. For our purposes, we can think of data frames as a spread sheet<!--^[2](#foot_dataframes)^-->. They have rows and columns and you can access values of the data frame by using certain commands.  
...
If you want to extract an entire column or an entire row, you just drop the idnex for the row or for the column, respectively. For example, to extract all values stored in the gear column, we drop the row index (e.g., `[, 10]` or `[, "gear"]`).
...
Another useful function is `mean()`, which takes a numeric vector as one of its arguments and returns the average of that vector as its output.
...
This is a small subset of the functions available in the `psych` package, and `psych` is only one package of over 11,000 on CRAN (as of 2018). This is not to mention the tens of thousands of packages hosted on online repositoris like GitHub. As Cory Costello noted during the R Bootcamp, the question in R is never *if* but *how*.
...
In the past, you would have to use file-type-specific functions to load data (e.g., `read.csv`, `read_excel`). The `rio` package streamlines this process by having a single import function (`import()`) that interprets the file type to be imported from its extension (e.g., `.csv`, `.xlsx`, `.sav`). As we did for `psych`, we will first need to install `rio`.
...
## Minihack 2: Creating and Coercing Variables

Assign the intger `50` to a variable called `x`. Use `typeof()` to confirm that it is an integer.

Assign the integer `50.` to a variable called `y`. Use `typeof()` to confirm that it is an integer.

Assign the word `hello` to a variable called `my_character`. Use `typeof()` to confirm that it is a character.

Assign the numbers `10`, `20`, and `30` to a vector called `my_vector`. 

## Minihack 3: Arithmetic Commands

In the console, use R to solve for $x$: $$x = \frac{(102 + 68) \times (3 + 2) + 1250}{50}$$

Add numbers to numeric vector,  

## Minihack 4: Loading and installing packages

Use the `capitalize()` function from the `Hmisc` package to capitalize `a_character_var` (the character variable created in Minihack 3). 
...
## Minihack 7: Importing Data

Download the [Star Wars' people dataset.](https://docs.google.com/spreadsheets/d/1AOYucRfucMR2TCBqzIdnlZoqXK4nyzPZCRzMztftCSg/export?format=xlsx). Import it into R.

## Minihack 8: Indexing Data Frames
