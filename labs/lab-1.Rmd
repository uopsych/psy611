...
After we have covered the lab content, we will move on to [Minihacks](#minihacks). Minihacks are small coding projects intended to test your knowledge of the day's material. The minihacks will be similar to but narrower in focus than the questions on your homework assignments. If you are able succesfully complete all of the minihacks, you should be well equipped to begin tackling the homeworks!
***
...
With data frames, you can extract a value by including `[row, col]` immediately after the object. For example, if we wanted to extract the value for the number of gears in the `Datsun 710` we could use `mtcars[3, 10]` to extract the value stored in the third row, tenth column.
...
If you want to extract an entire column, you can drop the index value for the row. Since you don't specify a given row, the computer assumes you want all of the values in the column. For example, to extract all values stored in the gear column, we drop the row index (e.g., `[, 10]` or `[, "gear"]`).
...
If we want to extract multiple columns (or multiple rows) we can also use vectors. For example, if we wanted number of gears and carboraters of the Datsun 710 and the Duster 360 we can use `[c("Datsun 710", "Duster 360"), c("gear", "carb")]` or `[c(3, 7), c(10:11)]`.
...
The `mean()` function and `sd()` function are two functions that you will end up using a lot. The former (`mean()`) takes a numeric vector as one of its arguments and returns the average of that vector as its output.

```{r, echo = FALSE, out.width = "350px"}
knitr::include_graphics(here::here("labs", "resources", "lab1", "functions", "lab1_mean.png"))
```

The latter (`sd()`) also takes a numeric vector as one of its arguments, but it returns the standard deviation of that vector as its output. 
...
Comments are pieces of code text that are not interpreted by the computer. In R we use the octothorpe/pound sign/hashtag (`#`) at the beginning of the line to denote a comment. The first and third line of code below are not evaluated, whereas the second and fourth line are.

```{r, echo = FALSE, out.width = "225px"}
knitr::include_graphics(here::here("labs", "resources", "lab1", "comments", "lab1_comment.png"))
```

Comments are mostly used to remind yourself (or other people) what a piece of code does **AND** why the code is written the way it is. Below is a piece of code that checks if a string is a valid phone number. 
...
```{r, echo = FALSE, out.width = "300px"}
knitr::include_graphics(here::here("labs", "resources", "lab1", "packages", "lab1_install.png"))
```
...
```{r, echo = FALSE, out.width = "300px"}
knitr::include_graphics(here::here("labs", "resources", "lab1", "packages", "lab1_load.png"))
```
...
```{r, echo = FALSE, out.width = "900px"}
knitr::include_graphics(here::here("labs", "resources", "lab1", "packages", "lab1_corr_test.png"))
```

Using `skew()`, we can look at the skew of all of the columns in `mtcars`.

```{r, echo = FALSE, out.width = "900px"}
knitr::include_graphics(here::here("labs", "resources", "lab1", "packages", "lab1_skew.png"))
```

We can use `t2d()` to calculate the Cohen's *d* for a t-value of 3.00 with 300 participants.

```{r, echo = FALSE, out.width = "250px"}
knitr::include_graphics(here::here("labs", "resources", "lab1", "packages", "lab1_t2d.png"))
```
...
For this example, we will be using the planets dataset from Star Wars. [The data can be downloaded here.](https://docs.google.com/spreadsheets/d/1W577PILjAkOSTNw7r85jjHrJaaXuOkiPuQcu2yA6F58/export?format=xlsx)
...
```{r, echo = FALSE, out.width = "250px"}
knitr::include_graphics(here::here("labs", "resources", "lab1", "importing_data", "lab1_install_rio.png"))
```

Second, we will need to load `rio`.

```{r, echo = FALSE, out.width = "260px"}
knitr::include_graphics(here::here("labs", "resources", "lab1", "importing_data", "lab1_load_rio.png"))
```
...
knitr::include_graphics(here::here("labs", "resources", "lab1", "importing_data", "lab1_import.png"))
```
...
```{r, echo = FALSE, out.width = "750px"}
knitr::include_graphics(here::here("labs", "resources", "lab1", "importing_data", "lab1_import2.png"))
```

To ensure it was read in properly, we can look at the first six rows of the imported dataset by using the `head()` function. 

```{r, echo = FALSE, out.width = "900px"}
knitr::include_graphics(here::here("labs", "resources", "lab1", "importing_data", "lab1_head.png"))
```

We can also look at the last six rows by using the `tail()` function.

```{r, echo = FALSE, out.width = "900px"}
knitr::include_graphics(here::here("labs", "resources", "lab1", "importing_data", "lab1_tail.png"))
```
...
## Minihack 1:  Creating an R Markdown
...
1. Add `x` to the length of `y` (*hint.* Use `length()`). Assign the result to a variable called `z`.

## Minihack 3: Functions

1. Assign the string `"I AM NOT YELLING"` to a variable called `exclamation`.

1. Use the function `tolower()` to convert every letter of `exclamation` to lower case. Assign the result to `exclamation`.
...
1. Using `mean()` and either the dollar sign (`$`) or square brackets (`[]`), calculate the average number of appearances using the `marvel_data`. Assign the result to a variable called `mean_appearances`.

1. Install and load the package `ggplot2` (or `tidyverse`).

1. If you succesfully completed the proceeding steps, you should be able to run the following code without an error. If you get an error, try to figure out why you are receiving and error. 
...
## Minihack 6: Rendering an R Markdown Document

1. Now that you have finished all of the minihacks, try rendering your `R Markdown` document by clicking `knit`. If it doesn't render correctly, try to figure out why not. 
