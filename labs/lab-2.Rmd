...
Here is a distribution with an outlier. 
...
```{r}
hist(corruption_std_rm_outlier$Corruption)
```
...
data_normal = rnorm(100, 5, sd = 2)
data_outlier = replace(data_normal, data_normal[25], 20)
```
...
1. Create a simple histogram of this distribution using the `hist()` function. 

```{r}
hist(data_outlier, main = "Distribution with outlier", xlab = "score")
```

2. Transform the scores using the `scale()` function. How many standard deviations above the mean does is the outlier?

3. Sort the dataset using the `sort()` function. (Again, remember you can look up help documentation for a function you're not familiar with using `?`). 

```{r}
data_outlier_sorted = sort(data_outlier)
```

4. Remove the outlier and save the resulting vector to an object called `data_outlier_removed`. 

```{r}
data_outlier_removed = data_outlier_sorted[-length(data_outlier_sorted)]
```

5. Recreate this histogram of the data without the outlier.

```{r echo=FALSE}
hist(data_outlier_removed, main = "Distribution with outlier removed", xlab = "score")
```
...
2. Create a histogram of the `Service` variable. Set the number of bins to `30`. (Feel free to use `ggplot` if you already familiar with it!) What do you notice about the shape of this distribution?
...
4. Based on what you observe about these group means, whow might this explain the shape of the distribution of the `Service` variable?
...
2. Use `cor()` to generate a correlation matrix for all the variables in `world_happiness_num`. Use listwise deleting by specifying `use = "complete.obs"`. 
