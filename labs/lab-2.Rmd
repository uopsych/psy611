...
3. Import the World Happiness data using the `rio` packageand save it to an object called `world_happiness`. (See [here](https://uopsych.github.io/psy611/labs/lab-1.html#importing_data_into_r){target="_blank"} for a refresher on importing data.)
...
```{r, echo=FALSE}
world_happiness <- import(here("/labs/resources/lab2/world_happiness_2015.csv"))
```
...
as.numeric(scale(simple_vector_outlier)) # the as.numeric() here just returns a vector instead of a matrix for formatting purposes
```
...
**I might use a code chunk to make some calculations:**
...
The mean of my vector is 5.78 with a standard deviation of 4.47. 


That's nice and all, but copying numbers over can be error prone AND if one thing changes about your analysis, then you need to complete change what you wrote up. Let's say you accidentally excluded one of your datapoints, you can add it back in and repeat the same code from before:

```{r}
my_new_vector <- c(my_vector, 5)

my_mean <- mean(my_new_vector)
my_mean
my_sd <- sd(my_new_vector)
my_sd
```

Actually, the mean of my vector is 5.7 with a standard deviation of 4.22.



**Alternatively you can embed your r code within your text:**
...
No matter, what you change about my_new_vector, the means is `r my_mean` and the standard deviation is `r my_sd`.


This will help make your code simpler, more reusable and less error-prone.
