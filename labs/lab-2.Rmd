...
You can also change the number of bins (i.e. bars) in your histogram using the `breaks` argument. Try different values and see how they affect the histogram. 
...
NOTE: `Country` is not `numeric`, it is a a **categorical** variable of type `character`. By default, the `describe()` function forces non-numeric variables to be numeric and attempts to calculate descriptives for them. These variables are marked with an asterisk (`*`). In this case, it doesn't make sense to describe `Country`, so we get a warning message and a bunch of `NaN`'s and `NA`'s for this variable. 
...
Again, note that we can't interpret the results for `Country` because it is a character variable.
...
```{r}
corruption_std <- import(here("labs/resources/lab2/corruption_standardized.csv"))
```
...
```{r}
world_happiness_sim <- import(here("/labs/resources/lab2/world_happiness_2015_sim.csv"))
```
...
For this minihack, we're going to use the `cor()` function from `{psych}` to create a correlation matrix of the entire `world_happiness` dataset. Be sure to use the original `world_happiness` data frame without the added variables.
...
```{r}
world_happiness_num <- world_happiness[, -1]
```
