...
We get a p-value of `0.001655805`. In other words, there is a ???. 

![](`r here("labs", "resources", "lab8", "lab8_4.png")`)

The result is multipled by `2` above because we want to be able to claim significance irrespective of whether the t-value is above or below the 50% value of `60`. As mentioned by Dr. Weston, two-tailed are, by far, the most common type of significance test. With the present data, we would not have been able to claim a significant result because the effect was in the opposite of our hypothesized direction. We specify `lower.tail = FALSE` here because we used the absolute value of the t-value. If you use a negative number, you will have to change `lower.tail = FALSE` to `lower.tail = TRUE`.
...
My advisor told me that I had multiple errors in my code. She told me the line numbers where the errors and what the correct values should be, but she thought it would be a good learning experience for me to try to solve them myself. I need your help. Fix the errors in the following code.

Error 1:
```{r, eval=FALSE}
ci_lower <- my_mean + ((my_sd / sqrt(my_n)) * qnorm(.025, 19, -.5))
ci_upper <- my_mean + ((my_sd / sqrt(my_n)) * qnorm(.975, 19, -.5))
```
...
## Minihack 3: Independant Sample's t-test

The researchers claim that the difference is significant at *p* = .043 using a two-tailed test, but you don't think it is. They were unwilling to share the data with you, but the following values were in the manuscript.

$\hat{\mu}_{Group 1}$ = 16.56

$\hat{\mu}_{Group 2}$ = 20.01

$\hat{\sigma}_{Group 1}$ = 5.44

$\hat{\sigma}_{Group 2}$ = 3.12

$n_{Group 1}$ = 10

$n_{Group 2}$ = 21

1. Calculate the t-value for the difference between the two means.

```{r}
mean_diff <- 16.56 - 20.01
se        <- sqrt(((5.44^2) / 10) + ((3.12^2) / 21))
t_val     <- mean_diff / se
```
...
df_num   <- ((5.44^2 / 10) + (3.12^2 / 21))^2
df_den_1 <- ((5.44^2) / 10)^2 / (10 - 1) 
df_den_2 <- ((3.12^2) / 21)^2 / (21 - 1)
df <- df_num / (df_den_1 + df_den_2)

# calculate p-value
pt(q = abs(t_val), df = df, lower.tail = FALSE) * 2
```

1. Why do you think the researchers got the p-value they did?

```{r}
pt(q = abs(t_val), df = df, lower.tail = FALSE)
```
