...
Unquote the following install line to install the papaja package, which is used for preparing APA style reports. Papaja stands for "Preparing APA Journal Articles"
...
2.  [Paired Samples *t*-test](#paired)
...
```{r}
#your code here:
sample_mean <- mean(data$cooperation)
mu <- 60
sigma_hat <- sd(data$cooperation)
N <- nrow(data)

t_stat <- (sample_mean - mu) / (sigma_hat / sqrt(N))
...
```{r}
lsr::oneSampleTTest(x = data$cooperation, 
               mu = 60)
```
...
## NOTE: Only t.test output is compatible with apa_print, the lsr package is not.

t_output  <- t.test(x = data$cooperation, mu = 60)

#Let's look at our output options
papaja::apa_print(t_output)
```
...
```{r}
#First I'll run my t-test again, but this time I'll save it to a variable:
pairedtoutput  <- t.test(x = chico_wide$grade_test2,
                         y = chico_wide$grade_test1,
                         paired = TRUE)
...
-   Practice using the output from 'apa_print' to write your results inline here:

> Performance significantly *increased* between Test 1 and Test 2 (`r apa_print(pairedtoutput)$full_result`)
