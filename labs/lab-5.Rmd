...
* Note that the output of`{dplyr}` functions is always a `data.frame`, which you'll see later on can be really useful when modelling and visualizing data. 
...
```{r}
world_happiness %>% 
  filter(happiness > mean(happiness, na.rm= TRUE) & gdp < mean(gdp, na.rm = TRUE))
```
...
* Sort rows in descending order by `happiness` and `gdp`

```{r}
world_happiness %>% 
  arrange(happiness, gdp)
```
