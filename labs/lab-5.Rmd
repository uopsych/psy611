...
The purpose of today's lab is to introduce you to the `tidyverse` as a framework for working with data structures in R.  We will mostly focus on data wrangling (particularly data transformation), including how to filter obervations, extract specific variable, generate new variables and summarize data. 
...
* Three qualities of the `tidyverse` are worth mentioning at the outset:
  ...
  2. The first argument of (basically) every function we'll review today is `data`. This is very handy, especially when it comes to pipinp (discussed [below](#pipes)).
...
For example, we can filter rows to retain only the observation for the United States.
...
\n
* You can combine multiple arguments to `filter()` with Boolean operators. The figure below from [*R for Data Science*](https://r4ds.had.co.nz/transform.html#logical-operators){target="_blank"} shows the complete set of these.
...
The first argument is `data`, followed by the names of the columns you want to subset. Note that you can use variable positions rather than their names, but this is usually not as useful. Let's go through some simple examples of common uses of `select()`...
...
* Produce a data frame that has only first-world countries with greater than average levels of freedom. Arrange the rows by `freedom` in descending order, and only display the `country` and `freedom` variables (in that order). How many observations are you left with?
...
* The `mutate()` function is most freqeuntly used to add columns to your data frame.

<center>
![](resources/lab5/manipulate_vars/mutate.png)
</center>

* `mutate()` requires data as its first argument, followed by a set of expressions defining new columns, separated by a comma. Let's take a couple examples...

* Create new variables
  + Note: New variables are added at the end of the data frame (scroll to the right to see them)

```{r}
world_happiness %>% 
  mutate(corruption_std = scale(corruption), # z-score `corruption` variable
         life_rounded = round(life, 0)) # round `life` variable to a whole number
```

* Change existing variables

When we imported our data, the `world` variable was categorized as an `integer`.
...
For the minihacks today, we will be working with the [`diamonds`](https://ggplot2.tidyverse.org/reference/diamonds.html){target="_blank"} dataset, which is built into R. Take a peek at the dataset with the following functions:
...
1. Take a look at the following chunk of code. If you're like me, this will make your head hurt. See if you can understand what it is trying to do. 
...
2. Re-write this code using pipes (` %>% `) so it is easier to read. Make sure you get the same result that you got when you ran the cluttered code. For an extra challenge, try making the filtering step more concise.
...
2. Which is more expensive on average, a `Fair`, `IF` (worst cut, best clarity) diamond, or an `Ideal`,`I1`(best cut, worst clarity)?
...
1. Calculate the following summary statistics for the `carat` variable for each `color` of diamond. Label your variables with the names in parentheses.

  * mean (`mean`)
  * standard deviation (`sd`)
  * n (`n`)
  * standard error of the mean (sem) (`sem`)
  * 95% confidence interval (lower and upper bounds) around the mean (using a *t* distribution). (`ci_lower` and `ci_upper`)
...
2. Repeat the same process above, but this time calculate summary statistics for the `carat` variable for each combination of `color` and `cut`. 
