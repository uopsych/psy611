...
The purpose of today's lab is to introduce you to the `tidyverse` as an alternative (and recommended) framework for working with data structures in R.  We will mostly focus on data wrangling (particularly data transformation), including how to filter obervations, extract specific variable, generate new variables and summarize data. 
...
For the minihacks today, we will be working with the `diamonds` dataset, which is built into R. Take a peek at the dataset with the following functions:
...
```{r eval = FALSE}
arrange(select(filter(diamonds, carat > 3 & carat < 4, cut == "Premium", color == "G" |  color == "H" | color == "I" | color == "J"), carat, color, price), desc(price))
```
