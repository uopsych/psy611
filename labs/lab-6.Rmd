...
ggplot(mapping = aes(x = Time, color = Diet))+
  geom_line(data = ChickWeight, aes(y = weight, group = Chick), alpha = 0.3)+ #first, we're plotting the data frame for the individual lines
  geom_point(data = ChickWeight, aes(y = weight, group = Chick), alpha = 0.3)+ #first, we're plotting the data frame for the individual lines
...
mtcars %>% 
  ...
  scale_fill_manual(values = c("4" = "green",
                               "6" = "blue",
                               "8" = "red"))+
  labs(x = "# Cylinders",
       y = "Horsepower",
...
mtcars %>% 
  ...
  ggplot(aes(x = cyl, y = hp))+
  geom_boxplot(aes(fill = cyl))+
  scale_fill_manual(values = c("4" = rgb(0, 100, 120, maxColorValue = 255),
