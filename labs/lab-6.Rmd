...
Your turn! Plot a histogram of the number of parrots each pirate has, split by whether they wear a headband. Label it appropriately and change the colors to ones that a pirate might like! I've started the code below, fill in the '?'
...
pirates %>%
  ...
  ggplot(aes(x = favorite.pirate, y = height)) +
    ...
    theme_bw() +
    # add labels 
    labs(title    = "Average height by favorite pirate",
         subtitle = "A ggplot plot",
         x        = "favorite pirate",
         y        = "Height (cm)",
         caption  = "Data from that `yarrr` package.")
...
Looks much better! Let's also rearrange the columns from shortest to tallest using `reorder(favorite.pirate, height)`. Here we are saying reorder the levels of the `favorite.pirate` variable by average height (`height`). *Note.* If we wanted to arrange from tallest to shortest, we would append `-` to the beginning of `height` (i.e., `reorder(favorite.pirate, -height)`).
...
Your turn! What if we wanted to distinguish the points by the pirates sex? As mentioned before, we can also specify an aesthetic mapping for `colour` (and `fill`). Let's map the point's colors to the pirates' sexes by including `aes(colour = sex))` in the `geom_point()` function.
