...
If we want to calculate the cumulative probability of getting a certain result, we would use the function `pbinom`. Cumulative probability might not make too much sense now, but by going through a couple examples it should help. For example, if we wanted to get the probability of getting at least `1` head when we are flipping two coinds, we would write `pbinom(q = 1, size = 2, prob = 1/2)`

```{r}
pbinom(q = 1, size = 2, prob = 1/2)
```

Essentially, the probability of getting `0` heads plus the probability of getting `1` heads in 



The function `pbinom()` takes the additional argument `lower.tail`, which defaults to `TRUE`. 
