...
The `aes()` function above tells `ggplot()` that we are providing it aesthetic information for the **canvas**. In this case, we are saying the **canvas** should have `age` on the x-axis. Other aesthetic mappings include `y`, `colour`, `size`, and `shape`, but we will discuss those later. Below, we print out the plot created by the code above.
...
And, finally, no plot is complete without proper labels. Fortunately, `ggplot2` has the `labs()` function built in for just this purpose.
