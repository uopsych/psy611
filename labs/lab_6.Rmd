...
Below is the **data set** `pirates` from the `{yarrr}` package. All of the plots in today's lab, excluding the Minihacks, we use the `pirates` data set.
...
We can pipe (`%>%`) that **data set** to the `ggplot()` function to create the **canvas** of our plot. Add `x = age` inside the `ggplot()` function.
...
We can change the way the **canvas** looks by using a suite of functions that start with the `theme_` prefix. I like `theme_bw()`, but other choices include `theme_gray()`, `theme_minimal()`, and `theme_classic()`.
