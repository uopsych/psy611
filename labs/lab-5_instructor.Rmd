...
# add mini-hacks that cover earlier material
...
You can download the .Rmd file [here](lab-5.Rmd).
...
* The `==` we just used is an example of a comparison operator that tests for equality. The other comparison operators available are :
  ...
  + `!=` (not equal to)
  + `%in%` (tests whether the element on left side of `%in%` is inside a vector on the right side)
...
#why is this returning a different output than the output from the code above?
#Because the mean of gdp is computed on the data frame that is already filtered for above avg happiness
world_happiness %>% 
  filter(happiness>mean(happiness, na.rm = TRUE)) %>%
  filter(gdp<mean(gdp, na.rm = TRUE))

#Let's do this in two steps:
above_avg_happiness <- world_happiness %>%
  filter(happiness > mean(happiness, na.rm = TRUE))

#this is the mean of gdp on the unfiltered world_happiness dataframe:
mean_gdp_unfiltered <- mean(world_happiness$gdp, na.rm = TRUE)
mean_gdp_above_avg_happy <- mean(above_avg_happiness$gdp, na.rm = TRUE)

#These two are not the same! In the first command of this code chunk, the means for happiness and gdp are calculated before any filtering happens!
```
...
* We can also determine the place where new variables should be added in the dataframe with the argument `.before = [column name]` or `.after = [column name]`. However, this refers to the placement of all new variables as they are created within a single `mutate()` function: 
...
1. Calculate the summary statistics listed below for the `carat` variable for each `color` of diamond. Give your summary variables the names indicated in parentheses.
  ...
  * 95% confidence interval (lower and upper bounds) around the mean (calculated using a *t* distribution). (`ci_lower` and `ci_upper`)
    + Hint: Refer to [this slide from class](https://uopsych.github.io/psy611/lectures/09-sampling.html#/29){target="_blank"} for an example
