...
1. [Paired samples *t*-test in R](#paired)
1. [Effect size](#effsize)
...
library(pwr) # for calculating the power for a t-test
```
...
# Paired samples *t*-test in R {#paired}
...
As you can see, both methods produce the same value for the standard deviation of the difference scores. 
...
Finally, before we calculate our *t*-statistic, we have to calculate the standard error. 
...
As always, we can calculate a *p*-value for our *t*-statistic using the `pt()` function. 
...
And we can also calculate a Cohen's d in exactly the same way as the one-sample and independent samples *t*-test. If we divide the mean of the difference scores by the standard deviation of the differences scores we get the Cohen's d (i.e., the standardized difference scores). 
...
Note that in the example above using the `pairedSamplesTTest()` function, we used the wide format of the data. When using wide data, you enter a one-sided formula that contains your two repeated measures conditions (e.g. `~ grade_test2 + gradte_test1`).

The `pairedSamplesTTest()` function can also be used with long data. In this case, you must use a two-sided formula of the form `outcome ~ group` and specify the name of the ID variable. Note that the group variable must be a factor. 
...
Instead of using the `{lsr}` functions, we can also use the more standard `t.test()` function from the `{stats}` package to run a paired samples *t*test. 
...
Cohen's D for a paired samples *t*-test is calculated as follows:

$$ d = \frac{\bar \Delta}{\hat \sigma_\Delta} $$

where $\hat \sigma_\Delta$ is the standard deviation of the difference scores

Paired samples Cohen's D can be calcuated in R with the `cohensD` function from `{lsr}`. 
...
```{r}
# wide format
ggpaired(chico_wide, cond1 = "grade_test1", cond2 = "grade_test2",
...
3. Let's say you're planning to replicate the study from minihack 1. Further the change the code above to estimate the minimum number of participants required to detect an effect of the same size with 80% power. *Note.* You will want to change the `type` argument back to paired.
...
3. Calculate the confidence interval. What does it mean?
