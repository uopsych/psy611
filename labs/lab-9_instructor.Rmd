...
* For now, we are going to work with `chico_wide`. Later on in the lab we will discuss more about how to deal with data that is in wide or long format.
...
And we can also calculate a Cohen's d in exactly the same way as the one-sample and independent samples *t*-test:
...
The within-subjects Cohen's D can be calcuated in R by using the `cohensD` function from `{lsr}` with the argument `method = "paired"`. 
