...
* Create the following two new variables (note that some of these variables were created in question 5.2. See the instructions for that questionz to ensure that variables are properly defined):
...
lenient. After all, some countries in the survey are quite well-off and would have good reasons for reporting higher than average levels of happiness. If the AAE is robust, it should show up even for countries that have less compelling reasons to be happy. Repeat the your statistical, but now separately for countries in the four development categories. Report all test statistics and _p_-values.
