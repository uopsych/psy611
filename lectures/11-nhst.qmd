...
data.frame(
  ...
  mutate(
    X = map(n, ~x_vals),
    density = map(n, ~dnorm(x = x_vals, m = 100, s = 15/sqrt(.x))),
...
```{r student null and alt, echo = F, fig.height = 8}
...
[Once the critical value and alternative value is established, we can determine the location of the critical value in the alternative distribution.]{style="font-size:30px;"}
...
```{r}
#| code-fold: true
means = data.frame(means = seq(101, 115, .2)) %>%
