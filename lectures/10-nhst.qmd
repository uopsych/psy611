...
<span style="font-size:30px;line-height: .5em">The normal distribution assumes we know the population mean and standard deviation. But we don’t. We only know the sample mean and standard deviation, and those have some uncertainty about them. </span>
...
<span style="font-size:30px;">At each sample size, draw 5000 samples from known population ( $\mu = 0$ , $\sigma = 1$ ).<br>Calculate CI for each sample using $s$ and record whether or not 0 was in that interval.<br>Calculate CI using for each sample using $\sigma$.</span>
...
<span style="font-size:30px;">Instead of calculating the probability of our statistic, we calculate the probability of our statistic *or more extreme* under the null.<br><br>The probability of success on 10 trials out of 12 or more extreme is `r round((1-pbinom(10,12,.5))*2,2)`.</span>
...
<span style="font-size:30px;">As we have more trials...</span>
...
<span style="font-size:30px;">... and more trials...</span>
...
<span style="font-size:30px;">If our measure was continuous, it would look something like this.</span>
...
<span style="font-size:30px;">The mean of the sampling distribution = the mean of the null hypothesis<br><br>The standard deviation of the sampling distribution:</span>
...
<span style="font-size:30px;">We have a normal distribution for which we know the mean (M), the standard deviation (SEM), and a score of interest ( $\bar{X}$ ).<br><br>We can use this information to calculate a Z-score; in the context of comparing one mean to a sampling distribution of means, we call this a **Z-statistic**.</span>
...
<span style="font-size:30px;">more NHST</span>
