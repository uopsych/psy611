...
### Covariation 
Sort of like the sums of squares of two variables
...
set.seed(101019) # so we all get the same random numbers
mu = c(3, 4)
Sigma = matrix(c(.9, .6, .6, .9), ncol =2) #diagonals are reliabilites, off-diagonals are correlations
data = mvrnorm(n = 50, mu = mu, Sigma = Sigma)
data = as.data.frame(data)
colnames(data) = c("x", "y")
data[51, ] = c(1.5, 5.5)
...
- Trait impulsivity is associated with BMI ([Terracianno et al., 2009](https://www.ncbi.nlm.nih.gov/pubmed/19414622)).

- But two of the items used to measure impulsivity are "I am able to control my cravings" and "I rarely overindulge." 

- What would the correlation look like if these items were removed?

---

Data source: [sapa-project.org](sapa-project.org), scales developed by [Condon (2017)](https://sapa-project.org/research/SPI/SPIdevelopment.pdf).

```{r, echo = FALSE}
new_sc_keys = list(spi.keys$SelfControl[-which(spi.keys$SelfControl %in% c("q_56", "q_1590"))])
keys = c(spi.keys, "new" = new_sc_keys)
scored_spi = scoreItems(keys, spi)
data_spi = as.data.frame(cbind(spi,scored_spi$scores))
set.seed(100919)
```

```{r, warning=FALSE, message=FALSE}
data_spi %>% ggplot(aes(x = SelfControl, y = exer)) + geom_jitter(alpha = .2) + geom_smooth(method = "lm", se = F) 
```
---
...
```{r, warning=FALSE, message=FALSE}
data_spi %>% ggplot(aes(x = SelfControl, y = exer)) + geom_jitter(alpha = .2) + geom_smooth(method = "lm", se = F) + geom_smooth(aes(x = new), method = "lm", se = F, color = "red")
```
...
```{r, fig.height = 5}
data %>% ggplot(aes(x=x, y=y, color = gender)) + geom_point() + geom_smooth(method = "lm", se = FALSE)
```
---
...
- **Point-biserial correlation coefficient**
    - Applies when Y is binary.
        - NOTE: This is not an appropriate statistic when you artificially<sup>1</sup> dichotomize data.
...
.footnote[
<sup>1</sup> It is almost [never appropriate](readings/Cohen_1983.pdf) to artifically dichotomize data.
]
