...
I just discovered this [nice blog post](https://richarddmorey.medium.com/power-and-precision-47f644ddea5e) about power & confidence intervals.
...
-   [increase $\mu_1$]{style="font-size:30px;"}
-   [decrease $CV_0$]{style="font-size:30px;"}
-   [increase $N$]{style="font-size:30px;"}
-   [reduce $\sigma$]{style="font-size:30px;"}
:::
:::::

------------------------------------------------------------------------

[Often the most challenging part of a power analysis is to settle on an effect size.]{style="font-size:30px;"}

-   [Past research can often provide some guidance, especially if a meta-analysis is available.]{style="font-size:30px;"}

    -   [Note that these are often over-estimates]{style="font-size:30px;"}.

-   [Sometimes a field might have standards regarding what counts as a meaningful effect (e.g., minimal clinically important difference).]{style="font-size:30px;"}

-   [Lacking this information, we can settle for more abstract benchmarks or rules of thumb about what are “small,” “medium”, and “large” effects.]{style="font-size:30px;"}

    -   [But as we discussed, these benchmarks need to be contextualized in real-world outcomes.]{style="font-size:30px;"}

    -   [Complicating these choices, effect sizes come in a variety of metrics.]{style="font-size:30px;"}
...
[Let's make some assumptions. If psychology studies use $\alpha = .05$ and are good at achieving adequate power ( $1-\beta = .80$ ), how does the choice of hypothesis affect PPV?]{style="font-size:30px;"}
...
[However, there are many reasons to think that psychology studies are underpowered. Typical power may be closer to .5 or even worse.]{style="font-size:30px;"}
...
[In addition to being under-powered, there's recognition that research practices have inflated family-wise error, making $\alpha$ much bigger than it should be.]{style="font-size:30px;"}
...
[click [here](http://p-curve.com/) for lots of extra information!]{style="font-size:30px;"}
...
[Let's simulate what would happen if we repeated an experiment 10,000 times in a world in which the null hypothesis was true...]{style="font-size:30px;"}

------------------------------------------------------------------------

### $H_0$ true $(N = 30, \mu = 0, \sigma = 1)$
...
[But note: what's an important assumption in these simulations?]{style="font-size:30px;"}
...
se = moe/tcv
tstat = X/se
p1 = pt(tstat, df = N-1, lower.tail = F)*2
...
se = moe/tcv
tstat = X/se
p2 = pt(tstat, df = N-1, lower.tail = F)*2
```

**Example:** Did the second study replicate the first?
...
| $\bar{X}$ |           30           |           10           |
|    95% CI |       \[5, 35\]        |       \[-1, 21\]       |
|       $p$ | `r papaja::printp(p1)` | `r papaja::printp(p2)` |
...
[Anderson & Maxwell [(2016)](../readings/anderson_2016.pdf)]{style="font-size:30px;"}

------------------------------------------------------------------------

Ultimately, much of the problems, both pre-2011 and since, have stemmed from an over-reliance on *p*-values ([Anderson, 2019](../readings/anderson_2019.pdf)).
