...
.left-column[
.small[
To determine the probability of a Type II error we must specify a value for the alternative hypothesis.  We will use the sample mean of `r x`.

In the long run, if psychology samples have a mean of `r x` ( $\sigma = `r s`$, $N = `r n`$ ), we will correctly reject the null with probability of `r round(power,2)` (power).  We will incorrectly fail to reject the
...
$$Z_1 = \frac{`r round(cv,2)` - `r x`}{\frac{`r s`}{\sqrt{`r n`}}} = `r round(z_a, 2)`$$

The proportion of the alternative distribution that falls below that point is the probability of a Type II error (.39); power is then .61.
```{r CHANGE ME}
pnorm(-.54)
```
