...
You can download an RMarkdown file of today's lecture/lab [here](lab-7.Rmd).
...
With the `gt::gt` function, you can add a table caption, change the column names, and format the table so that it's a little easier to read.
...
But we can do even more! Let's rename our MPG columns and put them under a single banner.
...
But really, summary statistics would be much preferable to what I have here. Let's combine the `gt` package with the `gtsummary` package for some useful descriptives.
