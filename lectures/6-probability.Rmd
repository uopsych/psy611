...
```{r}
X %>% ggplot(aes(x = num, y = coin)) + geom_line() + geom_hline(aes(yintercept = .5), color = "red", linetype = 2) + scale_x_continuous("Number of tosses") + scale_y_continuous("Proportion of heads", breaks = seq(0, 1, .1), limits = c(0,1)) + ggtitle("Proportion of heads as a function of number of tosses")
```
...
```{r}
X %>% ggplot(aes(x = num, y = die)) + geom_line() + geom_hline(aes(yintercept = 1/6), color = "red", linetype = 2) + scale_x_continuous("Number of rolls") + scale_y_continuous("Proportion of sixes", breaks = seq(0, 1, .1), limits = c(0,1)) + ggtitle("Proportion of sixes as a function of number of rolls")
```
...
```{r}
X %>% ggplot(aes(x = num, y = scrabble)) + geom_line() + geom_hline(aes(yintercept = .01), color = "red", linetype = 2) + scale_x_continuous("Number of draws") + scale_y_continuous("Proportion of Z tiles", breaks = seq(0, 1, .1), limits = c(0,1)) + ggtitle("Proportion of Z tiles as a function of number of draws")  + theme_gray(base_size = 20)
```
...
"The probability that Mike has his phone with him today is .2, or 20%."

<p>&nbsp;</p>

At one level, we all have an intuitive feel for what this means and understand it to tell us that Mike likely does not have his phone.
...
I either do or I do not have my phone with me today (there are only two outcomes in the sample space); it makes no sense to say that I have 20% of my phone with me today and the event is not repeatable (having my phone today) in the strict sense, so the “long run” doesn’t seem to apply.

<p>&nbsp;</p>

Instead, we have to say something like, on days like today (which can be repeated), the probability that Mike has his phone with him is .2.  
...
set.seed(101719)
TO = matrix(NA, ncol = 2, nrow = 10000)
for(S in 1:10000){
  TO[S,1] = S
  TO[S,2] = rbinom(n = 1, size = S, prob = .7)/S
}
TO = as.data.frame(TO); names(TO) = c("num", "prop")
label = data.frame(x = rep(9000,3), y = c(.8, .65, .45), 
                label = paste("Model", c("A", "B", "C")))

TO %>%
  ggplot(aes(x = num, y = prop)) + 
  geom_line() + 
  geom_hline(aes(yintercept = .7), color = "red", linetype = 2) + 
  geom_hline(aes(yintercept = .75), color = "red", linetype = 2) + 
  geom_hline(aes(yintercept = .4), color = "red", linetype = 2) + 
  geom_text(aes(x=x, y=y, label=label), data = label, color = "blue") +
...
```{r, warning = FALSE, message = FALSE, echo = FALSE, fig.height=6}
set.seed(101719)
TO = matrix(NA, ncol = 4, nrow = 10000)
for(S in 1:10000){
  TO[S,1] = S
  TO[S,2] = rbinom(n = 1, size = S, prob = .5)/S
  TO[S,3] = rbinom(n = 1, size = S, prob = .42)/S
  TO[S,4] = rbinom(n = 1, size = S, prob = .39)/S
}
TO = as.data.frame(TO); names(TO) = c("num", paste0("prop",1:3))

TO %>%
  ggplot(aes(x = num)) + 
  geom_line(aes(y=prop1)) + 
  geom_line(aes(y=prop2)) + 
  geom_line(aes(y=prop3)) + 
  scale_x_continuous("Number of events") + 
  scale_y_continuous("Proportion of target outcomes", 
                     breaks = seq(0, 1, .1), limits = c(0,1)) + 
  ggtitle("Proportion of target outcomes as a \nfunction of number of events") +
