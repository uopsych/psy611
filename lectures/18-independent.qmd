---
title: "Independent-samples _t_-tests"
format: 
  revealjs:
    multiplex: true
    slide-number: true
...
-   Due next Monday (9 am).
-   All quiz and homework revisions are due next Friday at 9am
...
[The bottom part, $N_1+N_2-2$ is the degrees of freedom for an independent sample *t*-test. The total number of quantities ( $N_1+N_2$ total scores) minus the number of constraints (2 means).]{style="font-size:30px;"}

[Each variance is weighted based on its sample's contribution to degrees of freedom.]{style="font-size:30px;"}
...
[At this point, the procedure of hypothesis testing goes back to normal.We have our sampling distribution under the null hypothesis, which is a *t* distribution defined by]{style="font-size:30px;"}

$$\mu = 0 \\$$ $$\sigma_D = \sqrt{\frac{(N_1-1)\hat{\sigma^2_1} + (N_2-1)\hat{\sigma^2_2}}{N_1+N_2-2}} \sqrt{\frac{1}{N_1} + \frac{1}{N_2}}$$ $$df = N_1 + N_2 - 2$$

[In this case, we're using $\sigma_D$ the same way we use $\sigma_M$, we just change the notation to reflect our interest in a difference in means, rather than the mean itself.]{style="font-size:30px;"}
...
::::: columns
::: {.column width="50%"}
$$\bar{X}_1 = `r round(stats$Female[1, "mean"],2)` $$ $$\hat{\sigma}_1 = `r round(stats$Female[1, "sd"],2)` $$ $$N_1 = `r round(stats$Female[1, "n"])` $$
:::

::: {.column width="50%"}
$$\bar{X}_2 = `r round(stats$Male[1, "mean"],2)` $$ $$\hat{\sigma}_2 = `r round(stats$Male[1, "sd"],2)` $$ $$N_2 = `r round(stats$Male[1, "n"])` $$
:::
:::::
...
$$\mu = 0$$ $$\sigma_D = \sqrt{\frac{(`r rn1`-1){`r rs1`}^2 + (`r rn2`-1){`r rs2`}^2}{`r rn1`+`r rn2`-2}} \sqrt{\frac{1}{`r rn1`} + \frac{1}{`r rn2`}}$$ $$= `r round(s,2)`\sqrt{\frac{1}{`r rn1`} + \frac{1}{`r rn2`}} = `r round(s_d,2)` $$
...
::::: columns
::: {.column width="30%"}
We calculate our test statistic and compare that to our critical value.
:::
...
::::: columns
::: {.column width="50%"}
[Your critical value (CV) is determined by your degrees of freedom and alpha.]{style="font-size:30px;"}

[In our example, we used $\alpha = .05$ and our df = $122+83-2 = 203$.]{style="font-size:30px;"}
...
::: {.column width="50%"}
$$CI_{\text{Difference}} = \\(18.31 -12.34) \pm 2.76(1.97)$$ $$[0.53, 11.41]$$
:::
:::::
...
In this case, you should use the pooled standard deviation as your estimate.

$$CI_{\text{Mean}} = {X} \pm \sigma_M(CV)$$ $$= {X} \pm \frac{\hat{\sigma}_p}{\sqrt{N}}(CV)$$

------------------------------------------------------------------------

$$= {X} \pm \frac{\hat{\sigma}_p}{\sqrt{N}}(CV)$$
...
$$18.31 \pm \frac{19.39}{\sqrt{122}}(1.98)$$ $$[14.83, 21.79]$$
:::
...
$$12.34 \pm \frac{19.39}{\sqrt{83}}(1.99)$$ $$[8.10, 16.58]$$
:::
:::::
