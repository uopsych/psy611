...
$$p(X|\mu, \sigma) = \frac{1}{\sqrt{2\pi\sigma}}exp[-\frac{(X-\mu)^2}{2\sigma^2}]$$ $$X\sim N(\mu,\sigma)$$
...
::::: columns
::: {.column width="30%"}
[The **probability density function** gives the height of the curve at a particular value for X.]{style="font-size:30px;"}

[Although these values communicate information about probability or likelihood, they are not probabilities.]{style="font-size:30px;"}
:::
...
::::: columns
::: {.column width="30%"}
[The area under the curve that lies between the mean (here 0) and a value of 1 is the probability of a score between 0 and 1.]{style="font-size:30px;"}

[As we shrink that area by moving X closer to the mean, that probability interpretation holds.]{style="font-size:30px;"}
:::
...
::::: columns
::: {.column width="30%"}
[As our interval shrinks closer and closer to 0, our area (probability) shrinks as well.]{style="font-size:30px;"}

[It can get vanishingly close to 0—essentially a point rather than an area.]{style="font-size:30px;"}

[The probability of that "point" is 0.]{style="font-size:30px;"}
:::
...
::::: columns
::: {.column width="30%"}
[We can keep shrinking the distance between Z and 0, never reaching 0, and still calculate an area.]{style="font-size:30px;"}

[It will be very, very small.]{style="font-size:30px;"}
:::
...
[Variables with quite different means and standard deviations can be standardized so that they can be compared in the same metric (standard deviation units). This allows statements such as "relative to the mean, I am more conscientious (e.g., $Z=2$) than I am extraverted (e.g., $Z=1$)."]{style="font-size:35px;"}

[I could not say, however, that I am twice as conscientious as I am extraverted.]{style="font-size:35px;"}

[All continuous distributions can be standardized, but if they are not normal to begin with, standardization will not make them so. *Standardization does not alter distribution shape.*]{style="font-size:35px;"}
...
-   [Given any score, we can calculate the probability of getting a value greater than that *z*-score. *(Or less than that z*-score.)]{style="font-size:35px;"}

-   [Given any two *z*-scores, we can calculate the probability of getting a value between these scores. *(Or outside those z*-scores)]{style="font-size:35px;"}

-   [Given a probability $p$, we can identify the *z*-score at which the proportion of scores below *(or above)* $p$ falls.]{style="font-size:35px;"}

-   [Given a probability $p$, we can identify the *z*-score at which the proportion of scores that fall above $-Z$ and below $Z$ is equal to $p$.]{style="font-size:35px;"}
...
::::: columns
::: {.column width="30%"}
[The $t$ distribution is used when data are normally distributed but we do not know the population mean or standard deviation.]{style="font-size:26px;"}

[This must be estimated from the sample and the added uncertainty produces a distribution with heavier tails. As sample size increases, the $t$ distribution converges on the normal distribution.]{style="font-size:26px;"}
:::
...
[Sometimes our interest will be in variability rather than means. We can make use of the $\chi^2$ distribution here. It has a single parameter, $\nu$, related to the sample size, $N$. There is an important relationship between the normal distribution and the $\chi^2$ distribution:]{style="font-size:30px;"}

$$\text{If } Z \sim N(0,1), \\\text{then }Z^2 \sim \chi^2_1 \\\text{ and } \sum_{i=1}^{N}Z^2 \sim \chi^2_N$$

[In other words, the sum of squared standard normal variables will have a $\chi^2$ distribution. Therefore, we can also assume that variance is $\chi^2$ distributed.]{style="font-size:30px;"}
...
[Another common question is related to comparing multiple variances. Suppose we conduct an experiment that has four different conditions. We expect that the means for these conditions will be different, but how can we know if they are different enough to claim that it is due to our experiment and not just random variability?]{style="font-size:30px;"}
...
[One approach to this question would start by taking the variability of the means, that is, variability between conditions:]{style="font-size:30px;"} $$\large s^2_{\text{between}}$$

------------------------------------------------------------------------

$$\Large s^2_{\text{between}}$$

[This variability will include any that was produced by the experimental conditions as well as any random variability. We have no way of separating those two parts. How can we know if the experiment-induced part is "big enough" to claim that the experiment "worked?"]{style="font-size:30px;"}
...
[We could compare it to the variability of the individual scores within the conditions:]{style="font-size:30px;"}
...
::: {.column width="50%"}
$$s^2_{\text{means}} = \frac{s^2_{\text{scores}}}{N}$$ $$Ns^2_{\text{means}} = s^2_{\text{scores}}$$
:::
:::::
