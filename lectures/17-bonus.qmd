...
trials_few <- 20
trials_many <- 100
true_effect <- 0.5  # Effect size in seconds

# Function to simulate one participant's data
simulate_participant <- function(n_trials, effect) {
  # Condition A: mean = 0, sd = 1
  cond_a <- rnorm(n_trials, mean = 0, sd = 1)
  # Condition B: mean = effect, sd = 1
  cond_b <- rnorm(n_trials, mean = effect, sd = 1)
  
  # Return participant means
  c(cond_a, cond_b)
}

# Simulate study with few trials
results_few <- replicate(n_participants, 
                        simulate_participant(trials_few, true_effect))

# Simulate study with many trials
results_many <- replicate(n_participants, 
                         simulate_participant(trials_many, true_effect))

# T-test results
t.test(results_few[1,], results_few[2,], paired = TRUE)$p.value
t.test(results_many[1,], results_many[2,], paired = TRUE)$p.value
```
...
simulate_cognitive_experiment <- function(
    ...
    participant_sd = 0.5,   # between-participant variability
  ...
  experiment_data <- expand_grid(
    ...
    select(participant, condition, trial, response, everything())
...
head(participant_effects)
```

------------------------------------------------------------------------

```{r}
# Basic paired t-test on participant means
t.test(
  participant_effects$B,
  participant_effects$A,
  paired = TRUE
)
```
