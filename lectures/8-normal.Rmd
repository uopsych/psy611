...
ggplot(data.frame(x = seq(-4, 4)), aes(x)) +
  ...
  stat_function(fun = function(x) dnorm(x) , 
                xlim = c(0, 1),
                geom = "area", fill = "red") + scale_x_continuous("Variable X") + scale_y_continuous("Density")+ggtitle("The normal curve")
```
...
ggplot(data.frame(x = seq(0, 1)), aes(x)) +
  ...
  scale_y_continuous("Density", limits=c(0,.4)) +ggtitle("Area under the normal curve between Z and 0")
...
ggplot(data.frame(x = seq(0, .001, by =.00005)), aes(x)) +
  ...
  geom_hline(aes(yintercept = 0), color = "red", linetype = "dashed")+
  scale_x_continuous("Z") +
  scale_y_continuous("Density") +ggtitle("Area under the normal curve between Z and 0")
...
**How is this useful?**
  - Given any s-score, we can calculate the probaiblity of getting a value greater than that z-score. *(Or less than that z-score.)*
...
Just like with the standard normal (Z) distribution, there are 1-1 relationships between probablility and "$t$-scores".
...
We assume the sample is a random selection from the population.
