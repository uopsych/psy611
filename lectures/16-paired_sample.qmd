...
se = sd(gulls$diff)/sqrt(nrow(gulls))
moe = se*qt(p = .975, df = nrow(gulls) - 1)
lower_ci = mean(gulls$diff) - moe
upper_ci = mean(gulls$diff) + moe
...
# Create plot
ggplot(gulls_long, aes(x = condition, y = seconds)) +
  # Add violin plots on opposite sides
  stat_halfeye(
    data = subset(gulls_long, condition == "At"),
    adjust = 0.5,
    width = 0.6,
    justification = 1.5,
    .width = 0,
  ...
  stat_halfeye(
    data = subset(gulls_long, condition == "Away"),
    adjust = 0.5,
    width = 0.6,
    justification = -0.5,
    .width = 0,
  ...
  geom_point(aes(group = GullID, color = condition),
             size = 2) +
  geom_line(aes(group = GullID),
            alpha = 0.2) +
  # Customize theme
